...
private:
    ...
    void random_delete(MyMesh* _mesh, int percent);
    void size_delete(MyMesh* _mesh, int percent);
    void angle_delete(MyMesh* _mesh, int percent);
    void flatness_delete(MyMesh* _mesh, int percent);
    ...
    /*struct Edges{
        int id;
        float size;
        bool operator<(const Edges& e1) const { return size < e1.size; }
    };*/
